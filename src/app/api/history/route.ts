...
export async function GET(req: Request) {
  ...
  const conversations = await sql`
    ...
    WHERE c.user_id = ${session.user.id} GROUP BY c.id ORDER BY c.updated_at DESC LIMIT 50
  `;
  const images = await sql`
    SELECT id, image_type as "imageType", topic, image_url as "imageUrl", created_at as "createdAt"
    FROM image_generations WHERE user_id = ${session.user.id} ORDER BY created_at DESC LIMIT 50
  `;
